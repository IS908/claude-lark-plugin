...
- **Session-health nudge (v1.4.0+, #190)**: Claude Code exposes NO programmatic `/compact`/`/clear` trigger (hooks observe only; Skill excludes built-ins; SDK has no compaction API — verified against docs 2026-06), and the main-loop session is shared with the operator's own terminal work, so autonomous clears are vetoed. Instead: the Stop hook (`enforce-lark-reply.mjs`) writes each session's exact context size (last assistant `usage`: input + cache_read + cache_creation; sidechain entries skipped) to `session-stats.json` keyed per session_id (pruned 48 h / 32 entries, atomic tmp+rename, failures never affect the hook verdict). `SessionHealthMonitor` (60 s tick, unref'd) nudges the OWNER via DM when heaviest-recent-session tokens > threshold AND inbound-idle > window AND queue quiet. Reminders follow an exponential ladder (`LARK_SESSION_NUDGE_COOLDOWN_MS` is the BASE, default 2 h: next nudge due base × 2^(n-1) after the n-th, i.e. 0/+2h/+6h/+14h undelayed, anchored on actual sends so delays shift instead of double-firing), max 4 per episode; episode closes on ≥30% measured drop (re-arm floor prevents instant post-compact re-nudge), re-arms on ≥25% regrowth, auto-resets after 24 h silence; 15 min retry backoff on DM failure never consumes a rung. Off by default (`LARK_SESSION_NUDGE_ENABLED`); requires `LARK_OWNER_OPEN_ID`. The nudge DM is deliberately NOT tracked in BotMessageTracker. `mcpServerInstructions` additionally tells the host session to delegate heavy multi-step work to subagents so tool transcripts stay out of main-loop history (prevention layer).
