...
- **Doc-comment events (v1.1.0+)**: subscribed via `drive.notice.comment_add_v1` in `src/channel.ts` (handler registration around line 843; dispatch via the pure `handleCommentEvent`). Bot must be a doc collaborator AND `is_mentioned=true` to trigger; the bot is automatically added as collaborator when @-mentioned in a comment. `reply_doc_comment` posts replies as the bot's app identity (tenant_access_token); requires "可评论" permission on the doc.
