...
| `LARK_SESSION_NUDGE_COOLDOWN_MS` | `7200000`（2 小时） | 指数退避阶梯的基数：第 n 条提醒未响应后，下一条在 `基数 × 2^(n-1)` 之后——默认值下累计节奏为 0 / +2h / +6h / +14h，每个 episode 最多 4 条。下次测量值下降 ≥30%（或低于阈值）时 episode 关闭；重新增长 ≥25% 时重新武装；阶梯静默 24 小时后自动重置 |
