...
| `LARK_IDENTITY_SESSION_MAX_SIZE` | `5000` | 调用者身份 LRU 缓存的最大条目数。文档评论事件（v1.1.1+）按 comment_id 分桶，所以同时参与多个文档协作的 bot 累积的条目数会高于纯 IM 时代。最小值 1（更小值被夹紧）；无上限——运维按内存预算（每条 ~80 字节）选择。超出容量时驱逐最旧条目。 |
