...
| `LARK_IDENTITY_SESSION_MAX_SIZE` | `5000` | Max number of live entries in the per-(chat,thread) caller-identity LRU. Doc-comment events (post-v1.1.0) key per-comment, so a busy bot collaborating in many docs can accumulate more entries than the legacy IM-only flow. Range [1, 1_000_000]; oldest entry evicted when capacity exceeded. |
