...
- **#159**: race protection for `react` / `download_attachment` requires inbound-id awareness (TTL cache or notification-meta plumbing). Until then, react-only / download-only responses to fast inbound messages may still see the MeMeMe linger up to ~6 min — same as v1.0.43 behavior for those tools. The much more common reply-based response path is fully race-protected.
