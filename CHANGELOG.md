...
- Reactions on bot messages that have aged out of `BotMessageTracker` (default size 500, configurable via `LARK_BOT_MESSAGE_TRACKER_SIZE`) are still silently dropped, but now log a breadcrumb to `debug.log` so the operator can see why. If you have a high-traffic deployment where users react to days-old bot cards, raise `LARK_BOT_MESSAGE_TRACKER_SIZE`.
