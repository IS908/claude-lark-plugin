...
- `hooks/enforce-lark-reply.mjs`: `writeSessionStats` called right after transcript load so every early-exit path still records; `hooks/test-enforce-lark-reply.mjs` gains a default tmp `LARK_SESSION_STATS_PATH` for ALL spawns (keeps the suite out of the real sidecar) + 9 new cases (S1–S9: exact sum, no-usage / sidechain-only no-file, corruption recovery, sidechain skip, unwritable-path verdict isolation, prune semantics incl. future-ts rejection, 32-entry count cap, loop-break skip, zeroed-usage fallback).
